...
def build_task_init():
    ...
    env['GCCPREFIX'] = os.path.join(os.environ['CONFIG_TOOLCHAIN_PATH'], os.environ['CONFIG_TOOLCHAIN_PREFIX'].strip('"'))
    ...
    try:
        env.ParseConfig('${{CC}} -v 2> {}'.format(str(Path('build/config/gcc_version.txt'))))
